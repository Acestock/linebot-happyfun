...
**主動行為（排程）**：遊戲逾時沒人玩會自動收攤；群組安靜超過 `IDLE_NUDGE_THRESHOLD_MINUTES`（預設 3 小時）且在台灣時間 09:00–22:00 之間，機器人會主動丟一個話題。所有主動推播受 `PUSH_RATE_LIMIT_PER_GROUP_PER_HOUR`（預設每群組每小時 2 則）保護，控制 LINE Push 費用。
