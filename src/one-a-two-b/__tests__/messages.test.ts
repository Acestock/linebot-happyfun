...
import { buildAllTimeTopThreeCard, buildLeaderboardCard, isOneATwoBLeaderboardCommand } from "../messages";
...
describe("buildLeaderboardCard", () => {
  it("prompts to play when nobody has played today, but still offers the top-3 button", () => {
    ...
    expect(json).toContain("每日 1A2B");
    expect(json).toContain("action=one_a_two_b_top3");
  });
